...
function getAccLevelByCol(data, countries, pot_acc_level){
  count = [];
  dict = [];
  ...
  for (var i=0; i<countries.length; i++){
    ...
    { 
      if( d["Country"] == countries[i] && d["Potential_Accident_Level"] == pot_acc_level)
      { 
        return d;
      } 
    })
    count[countries[i]] = filteredData.length;
  }
...
function makeBiVariateChart(dataset) {
  ...
  for (var i=0; i<pot_acc_levels.length; i++){
    pot_acc_by_col_dict[pot_acc_levels[i]] = getAccLevelByCol(dataset, countries, pot_acc_levels[i]);
  }
